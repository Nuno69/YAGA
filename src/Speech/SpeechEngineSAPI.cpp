...
SpeechEngineSAPI::SpeechEngineSAPI() : SpeechEngine(L"SAPI5"), voice(NULL)
{
    Init();
}
...
int SpeechEngineSAPI::Read(const wchar_t *str, int interrupt)
{
    ...
    // I do these flags so that the voice is more responsive. ASYNC means that the game's thread will not be blocked
    // when the speech is speaking, while SPF_ISNotXML disables the processing of Speech SYnthesis Markup Language
    // (SSML) which has no use here anyway.
