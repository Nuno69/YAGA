...
namespace GOTHIC_NAMESPACE {
	...
	struct zSmartMap {
		...
		{
			if (zinput->KeyToggled(KEY_LEFT))			Move(-1, 0);
			else if (zinput->KeyToggled(KEY_RIGHT))		Move(+1, 0);
			else if (zinput->KeyToggled(KEY_UP))		Move(0, +1);
			else if (zinput->KeyToggled(KEY_DOWN))		Move(0, -1);
			else if (zinput->KeyToggled(KEY_RETURN))	Details();
		...
		static void Details()
		{
			...
			if (orcs > 0)     message += std::format(L"Orcs: {}. ", orcs);
			if (monsters > 0) message += std::format(L"Monsters: {}. ", monsters);
