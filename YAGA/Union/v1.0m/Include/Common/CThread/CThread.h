...
namespace Common {
  ...
  class Switch {
    Event EventA, EventB; // ON, OFF
  public:
    Switch( const bool& isOn = false ) : EventA( isOn ? 1 : 0 ), EventB( isOn ? 0 : 1 ) {
    ...
    bool_t WaitOn( const int& milliseconds = INFINITE ) {
      return EventA.Wait( milliseconds );
    }
    ...
    bool_t WaitOff( const int& milliseconds = INFINITE ) {
      return EventB.Wait( milliseconds );
    }
