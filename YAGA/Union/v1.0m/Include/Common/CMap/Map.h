...
namespace Common {
  ...
  inline int Compare( const MapPair<KEY, VALUE>& pair, const CMP& value ) {
    return Compare( pair.GetKey(), value );
  }

  template<class KEY, class VALUE, class CMP>
  inline int Compare( const CMP& value, const MapPair<KEY, VALUE>& pair ) {
    return Compare( value, pair.GetKey() );
  }
